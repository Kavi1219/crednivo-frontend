...
export default function Loans() {
  ...
  return <div className="module-page loans-page">
    ...
    <section className="module-card">
      ...
      <div className="module-table-wrap desktop-data-table"><table className="module-table"><thead><tr><th>Loan</th><th>Customer</th><th>Cycle</th><th>Type</th><th>Principal</th><th>Collection</th><th>Outstanding</th><th>Next Due</th><th>Status</th><th>Created By</th><th>Action</th></tr></thead><tbody>{filtered.map(loan=><tr key={loan.id}><td><strong>{loan.id}</strong></td><td><div className="loan-customer-identity"><CustomerAvatar className="loan-customer-avatar" photo={customerPhotoById[String(loan.customerId)]} name={loan.customerName}/><div><CustomerProfileLink customerId={loan.customerId}>{loan.customerName}</CustomerProfileLink><small className="table-sub">{loan.customerId}</small></div></div></td><td><span className="soft-chip blue">{loan.cycle}</span></td><td>{loan.loanType}</td><td>{formatCurrency(loan.principal)}</td><td>{formatCurrency(loan.collectionAmount)}</td><td><strong>{formatCurrency(loan.outstanding)}</strong></td><td>{formatDate(loan.nextDueDate)}</td><td><span className={`soft-chip ${loan.status==='Closed'?'gray':loan.status==='Overdue'?'red':'green'}`}>{loan.status}</span></td><td>{loan.createdBy || 'Not recorded'}</td><td><IconButton size="sm" label={`View ${loan.customerName}`} onClick={()=>navigate(`/customers/${loan.customerId}`)}><Eye size={16}/></IconButton></td></tr>)}</tbody></table></div>
      <div className="mobile-data-list">{filtered.map(loan=><article className="mobile-data-card" key={loan.id}><div className="mobile-data-top"><div className="loan-customer-identity"><CustomerAvatar className="loan-customer-avatar" photo={customerPhotoById[String(loan.customerId)]} name={loan.customerName}/><div><strong><CustomerProfileLink customerId={loan.customerId}>{loan.customerName}</CustomerProfileLink></strong><small className="table-sub">{loan.id}</small></div></div><IconButton size="sm" label="View customer" onClick={()=>navigate(`/customers/${loan.customerId}`)}><Eye size={16}/></IconButton></div><div className="mobile-data-meta"><div><span>Cycle</span><strong>{loan.cycle} · {loan.loanType}</strong></div><div><span>Collection</span><strong>{formatCurrency(loan.collectionAmount)}</strong></div><div><span>Outstanding</span><strong>{formatCurrency(loan.outstanding)}</strong></div><div><span>Status</span><strong>{loan.status}</strong></div><div><span>Created By</span><strong>{loan.createdBy || 'Not recorded'}</strong></div></div></article>)}</div>
    </section>
  </div>;
}
